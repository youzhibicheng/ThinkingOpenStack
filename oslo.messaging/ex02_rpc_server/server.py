...
# blocking：用户调用start函数后，在start函数中开始请求处理循环，用户线程阻塞，处理下一个请求。直到用户调用了stop函数后，这个处理循环才退出。
# 消息的接受和分发处理都在调用start函数的线程中完成的。
# eventlet：在这种方式中，会有一个协程GreenThread来处理消息的接收，然后有其他不同二屌GreenThread来处理不同消息的分发处理。调用start的用户线程不会被阻塞。
# 在这里我们使用blocking方式。
